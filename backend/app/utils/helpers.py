...
def safe_parse_single_date(val):
    """Parsing tanggal aman dengan translasi bulan Indonesia & support Excel serial numbers."""
    if pd.isna(val):
        return pd.NaT
...
def validate_dates(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in df.columns:
        ...
        if any(ex in col_lower for ex in ['usia', 'age', 'uw_year', 'uy']):
            continue

        if any(keyword in col_lower for keyword in keywords):
            s_parsed = df[col].apply(safe_parse_single_date)

            # Penanganan tahun 2 digit (misal 13 -> 2013)
            mask_2digit = (s_parsed.dt.year >= 0) & (s_parsed.dt.year < 100)
            if mask_2digit.any():
