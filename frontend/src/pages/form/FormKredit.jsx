...
export default function FormKredit() {
  // Master Cedant untuk COB Kredit
  const creditCedants = [
    { code: 'ALL', name: 'Semua Cedant', is_all: true },
    { code: 'jamkridajabar', name: 'PT Penjaminan Kredit Daerah Jabar (Jamkrida Jabar)', prefix_premi: 'premi_jamkridajabar_credit', prefix_claim: 'claim_jamkridajabar_credit' },
    { code: 'askrida', name: 'PT Asuransi Bangun Askrida (Askrida)', prefix_premi: 'premi_credit_askrida', prefix_claim: 'claim_kredit_askrida' },
    { code: 'jakrejabar', name: 'PT Jasa Raharja Cabang Jabar (Jakre Jabar)', prefix_premi: 'premi_jakrejabar_credit', prefix_claim: 'claim_jakrejabar_credit' },
    { code: 'askrindo', name: 'PT Asuransi Kredit Indonesia (Askrindo)', prefix_premi: 'premi_askrindo_credit', prefix_claim: 'claim_askrindo_credit' },
    { code: 'jamkrindo', name: 'PT Jaminan Kredit Indonesia (Jamkrindo)', prefix_premi: 'premi_jamkrindo_credit', prefix_claim: 'claim_jamkrindo_credit' },
  ];
  ...
  const currentTableName = useMemo(() => {
    if (selectedCedant === 'ALL') {
      return selectedType === 'PREMIUM' ? 'all_premi_credit' : 'all_claim_credit';
    }
    const found = creditCedants.find(c => c.code === selectedCedant);
    if (found) {
      return selectedType === 'PREMIUM' ? found.prefix_premi : found.prefix_claim;
    }
    return selectedType === 'PREMIUM' ? `premi_${selectedCedant}_credit` : `claim_${selectedCedant}_credit`;
  ...
  const exactExcludedColumns = new Set([
    'no', 'id', 'remarks', 'unnamed', 'notes', 'trx', 'period', 'cob', 'created_at', 'cedant_name'
  ]);
  ...
  const fetchPeriodList = async (tableName) => {
    try {
      ...
      console.error('Gagal mengambil daftar periode kredit:', err);
      setPeriodList([]);
    }
  };
  ...
  useEffect(() => {
    fetchCreditData(currentTableName, page, limit, filterStatus, filterPeriod);
  ...
  const processedData = useMemo(() => {
    ...
    return dataList.map((row) => {
      ...
      columns.forEach((col) => {
        ...
        const isExcluded = exactExcludedColumns.has(colLower);
  ...
  const activeCedantObj = creditCedants.find(c => c.code === selectedCedant) || { name: selectedCedant };
  ...
  return (
    <div className="p-6 space-y-5 text-xs bg-slate-50 min-h-screen relative font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-lg md:text-xl font-bold text-slate-800 flex items-center gap-2 mt-0.5">
            ...
            <span>Bordero Data - COB Kredit</span>
          </h1>
          <p className="text-slate-500 text-[11px] mt-0.5">
            Monitoring data real-time transaksi Premi dan Klaim Lini Bisnis Kredit.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button
            type="button"
            onClick={() => fetchCreditData(currentTableName, page, limit, filterStatus, filterPeriod)}
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-2xs space-y-3.5">
        ...
        <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-3 pb-3 border-b border-slate-100">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="relative" ref={cedantDropdownRef}>
              ...
              {openCedantDropdown && (
                <div className="absolute left-0 mt-1.5 w-84 bg-white border border-slate-100 rounded-xl shadow-xl z-30 py-1.5 space-y-0.5 animate-in fade-in zoom-in-95 duration-100">
                  ...
                  {creditCedants.filter(c => !c.is_all).map((c) => (
                    <button
                      key={c.code}
                      type="button"
        ...
        {/* Row 2: Search, Filter Periode, Status, Limit */}
        ...
        <div className="flex items-center gap-2">
          {isAggregateView ? (
            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-indigo-50 border border-indigo-200/80 text-indigo-800 font-semibold">
              <Layers className="w-3.5 h-3.5 text-indigo-600" />
              <span>Tampilan Konsolidasi: Seluruh tabel fisik {selectedType === 'PREMIUM' ? 'Premi' : 'Klaim'} Kredit ({targetTablesCount} tabel) disatukan pada antarmuka ini.</span>
            </span>
          ) : (
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-2 text-xs text-slate-500">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-1 ml-2">
                  <button
                    type="button"
                    onClick={() => fetchCreditData(currentTableName, page - 1, limit, filterStatus, filterPeriod)}
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-2xl max-w-md w-full p-5 space-y-4 font-sans">
            ...
            <div className="flex items-center justify-between pb-2 border-b border-slate-100">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-slate-800 text-sm">Ekspor Data Bordero Kredit</h3>
              </div>
              <button
                type="button"
            ...
            <div className="space-y-3 text-xs text-slate-600">
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-200/60 space-y-1">
                ...
                <p className="text-[11px] text-slate-600">
                  Kategori: <strong className="text-slate-800">{selectedType === 'PREMIUM' ? 'Premi Kredit' : 'Klaim Kredit'}</strong>
                </p>
                <p className="text-[11px] text-slate-600">
