...
export default function UploadWidget({
  ...
  const [globalPeriodType, setGlobalPeriodType] = useState(() => {
    const saved = sessionStorage.getItem(SESSION_KEY);
    return saved
      ? JSON.parse(saved).globalPeriodType || "quarterly"
      : "quarterly";
  });
  ...
  const uniqueAvailableSheets = useMemo(() => {
    ...
    files.forEach((file) => {
      file.available_cobs?.forEach((cobItem) => {
        if (!sheetMap.has(cobItem.sheetName)) {
          sheetMap.set(cobItem.sheetName, cobItem);
        }
      });
    });
    ...
    setFiles((prev) =>
      prev.map((f) => {
        ...
        if (matched) {
          return {
            ...f,
            selectedSheet: matched.sheetName,
            cob: matched.cobCode,
          };
        }
        return f;
  ...
  const handleAddFiles = async (incomingFiles) => {
    ...
    const defaultCategoryForCedant = globalCategory || "";
    ...
    const newFileObjects = validFiles.map((file) => {
      ...
      if (
        ...
        !["Q1", "Q2", "Q3", "Q4", "TW1", "TW2", "TW3", "TW4"].includes(
          initialPeriod,
        )
      ) {
        initialPeriod = "TW1";
      ...
      return {
        ...
        name: file.name,
        size: file.size,
        category: defaultCategoryForCedant || detected.category,
    ...
    const inspectPromises = targetList.map(async (item) => {
      ...
      try {
        ...
        const finalCob =
          defaultCobForCedant === "CREDIT"
            ? "CREDIT"
            : detectedCobs[0]?.cobCode || "FIRE";
      ...
      prev.map((f, i) => {
        if (i === index) {
          if (field === "selectedSheet") {
            const matched = f.available_cobs?.find(
              (c) =>
                c.sheetName.trim().toLowerCase() ===
                String(value).trim().toLowerCase(),
            );
            return {
              ...f,
              selectedSheet: value,
              cob: matched ? matched.cobCode : f.cob,
            };
          }
          if (field === "cob") {
        ...
        <div className="lg:col-span-6 bg-slate-50/50 p-4 rounded-2xl border border-slate-200/70 flex flex-col justify-between space-y-2">
          ...
          <div className="flex items-center gap-2.5 flex-wrap">
            ...
            >
              ...
              <span>
                {selectedCedant
                  ? `Siap: ${selectedCedant.defaultCob}`
                  : "Menunggu Cedant"}
              </span>
            </div>
        ...
        <div className="flex items-center justify-between border-b border-slate-100 pb-3 flex-wrap gap-2">
          <div>
            ...
            <p className="text-[11px] text-slate-400 mt-0.5">
              Unggah berkas Excel mentah untuk diekstrak oleh pipeline ETL.
            </p>
          </div>
                ...
                <div className="flex items-center gap-2 border-r border-slate-200 pr-3">
                  ...
                  <div className="flex items-center bg-slate-50 border border-slate-200 rounded-lg p-0.5">
                    <select
                      ...
                      className="bg-transparent text-[10px] font-semibold outline-none px-1 text-slate-700 cursor-pointer"
                    >
                    ...
                    <button
                      ...
                      className="px-2 py-1 bg-blue-600 text-white text-[9px] font-bold rounded-md hover:bg-blue-700 transition-colors cursor-pointer"
                    >
                      Apply
                    </button>
                  </div>
                  ...
                  <div className="flex items-center bg-slate-50 border border-slate-200 rounded-lg p-0.5">
                    <select
                      ...
                      onChange={(e) => setGlobalSelectedSheet(e.target.value)}
                      className="bg-transparent text-[10px] font-semibold outline-none px-1 text-slate-700 cursor-pointer max-w-[160px]"
                    >
                      <option value="">Pilih Sheet / COB...</option>
                      {uniqueAvailableSheets.map((item, idx) => (
                        <option key={idx} value={item.sheetName}>
                          {item.cobCode} ({item.sheetName})
                        </option>
                      ))}
                    </select>
                    <button
                      type="button"
                      onClick={handleApplyAllSheet}
                      disabled={!globalSelectedSheet}
                      className="px-2 py-1 bg-blue-600 text-white text-[9px] font-bold rounded-md hover:bg-blue-700 transition-colors cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed"
                    >
                      Apply
                    </button>
                  </div>
