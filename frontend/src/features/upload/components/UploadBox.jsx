...
export default function UploadBox({
  ...
  // VALIDASI FORM: Cek apakah SEMUA file sudah memilih Tipe, Periode, dan Tahun (4 digit)
        ...
        <div className="mt-4 flex-1 animate-in fade-in duration-300 space-y-2">
          <div className="flex items-center justify-between">
            ...
            <span className={`text-[10px] font-semibold ${isFormValid ? 'text-emerald-600' : 'text-amber-500'}`}>
              {isFormValid ? '✓ Informasi Lengkap' : '* Lengkapi Tipe, Periode & Tahun'}
            </span>
          </div>
                ...
                <div className="flex items-center gap-2 shrink-0 bg-slate-50/80 p-1.5 rounded-lg border border-slate-100/80">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex flex-col">
                      <span className="text-[8px] uppercase font-bold text-slate-400 ml-1 mb-0.5 tracking-wider">Tipe (COB)</span>
                      <select
                        value={fileObj.category || ''}
                        onChange={(e) => onUpdateFile(index, 'category', e.target.value)}
                        className={`bg-white border border-slate-200 rounded-md text-[11px] px-2 py-1 w-[100px] focus:ring-1 focus:ring-indigo-500 outline-none shadow-sm font-medium cursor-pointer ${
                    ...
                    <div className="flex flex-col">
                      ...
                      <select
                        ...
                        className={`bg-white border border-slate-200 rounded-md text-[11px] px-2 py-1 w-[80px] focus:ring-1 focus:ring-indigo-500 outline-none shadow-sm font-medium cursor-pointer ${
