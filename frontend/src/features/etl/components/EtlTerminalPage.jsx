  ...
  useEffect(() => {
    ...
    async function runExecution() {
      ...
      const payloadItems = files.map(f => {
        const cleanCob = (f.cob || 'FIRE').toLowerCase();
        const targetTable = `${(f.category || 'premi').toLowerCase()}_${(cedantCode || 'tripakarta').toLowerCase()}_${cleanCob}`;
      ...
      payloadItems.forEach((item, idx) => {
        addLog(
          `[Item ${idx + 1}] File ID: ${item.file_id} | COB: ${item.override_cob} | Periode: ${item.kuartal} ${item.tahun} | Sheet: "${item.target_sheet}"`,
          'file-ready',
          item
        );
      ...
      try {
        ...
        if (isSingle) {
          ...
          if (response.data?.status === 'success') {
            ...
            addLog(`✓ Sukses: ${rows.toLocaleString('id-ID')} baris masuk ke tabel "${response.data.detail?.target_table}".`, 'success');
            ...
            finalFilesReport.push({
              ...
              rows: rows,
              table: response.data.detail?.target_table,
              status: 'success',
          ...
          if (response.data?.status === 'completed') {
            response.data.batch_details?.forEach((detail, idx) => {
              ...
              if (isSuccess) {
                ...
                addLog(`✓ [${detail.file_id}] Sheet "${detail.target_sheet}" sukses: ${detail.rows_inserted.toLocaleString('id-ID')} baris tersimpan.`, 'success');
              } else {
              ...
              finalFilesReport.push({
                ...
                period: `${originalFile?.period} ${originalFile?.receivedDate}`,
                rows: detail.rows_inserted || 0,
                status: detail.status,
                error: detail.error_message || null,
                logMessage: isSuccess 
                  ? `Berhasil diekstrak (${detail.rows_inserted} baris).`
